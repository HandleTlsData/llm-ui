...
function MainChat() 
{
    ...
    const [activeChatHistory, setActiveChatHistory] = useState<ChatHistory[]>([]);
    ...
    const getChatHistory = async (chatID: number) => 
    {
        try 
        {
            ...
            setActiveChatHistory(response.data);
        } 
        catch (error) 
        {
    ...
    const sendChatMessage = async () => 
    {
        try 
        {
            const outgoingChatEntity: ChatHistory = { local: true, text: textAreaData.text, image: '' };
            setActiveChatHistory((activeChatHistory) => [...activeChatHistory, outgoingChatEntity]);
      
            const data = { token: localStorage.getItem('authtok'), chatID: activeChatID, message: textAreaData.text, 
                image: ''
            ...
            const incomingChatEntity = {local: false, text: response.data.text, image: response.data.image };
            setActiveChatHistory((activeChatHistory) => [...activeChatHistory, incomingChatEntity]);
  ...
  <nav className="size-full flex flex-col">
    ...
    <div className="h-full">
      <ul className="space-y-1.5 p-4">
        <li onClick={(e)=>{createNewChat();e.preventDefault();}}>
        ...
        {chatList.length > 0 && chatList.map((entity, index) => (
            <li key={index} onClick={(e)=>{switchChat(entity); e.preventDefault()}}>
                <a className="flex items-center gap-x-3 py-2 px-3 text-sm text-gray-700 rounded-lg hover:bg-gray-100 focus:outline-none focus:bg-gray-100 dark:text-neutral-400 dark:hover:bg-neutral-900 dark:hover:text-neutral-300 dark:focus:bg-neutral-900 dark:focus:text-neutral-300" href="#">
                <svg xmlns="http://www.w3.org/2000/svg" onClick={(e)=>{deleteChat(entity);e.preventDefault();e.stopPropagation()}} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-trash"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/></svg>            
                Chat {entity}
                </a>
            </li>
        ))}
    ...
    <div className="mt-auto">
      ...
      <div className="p-4 border-t border-gray-200 dark:border-neutral-700">
        <a className="flex justify-between items-center gap-x-3 py-2 px-3 text-sm text-gray-700 rounded-lg hover:bg-gray-100 focus:outline-none focus:bg-gray-100 dark:hover:bg-neutral-900 dark:text-neutral-400 dark:hover:text-neutral-300 dark:focus:text-neutral-300" href="#">
          {myUsername}
          <svg className="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4"/><polyline points="10 17 15 12 10 7"/><line x1="15" x2="3" y1="12" y2="12"/></svg>
        </a>
      </div>
    </div>
  </nav>
...
{activeChatID && (<div className="relative h-screen w-full lg:ps-64">
  <div className="py-10 lg:py-14">
    ...
    {activeChatHistory.map((entity, index) => {
          if(entity.local)
          {
            return (
                ...
                <div className="max-w-4xl px-4 sm:px-6 lg:px-8 mx-auto">
                  <div className="max-w-2xl flex gap-x-2 sm:gap-x-4">
                    ...
                    <div className="grow mt-2 space-y-3">
                      {entity.image && (<img className="w-full object-cover" src="" alt="Deep Learning" />)}

                      <p className="text-gray-800 dark:text-neutral-200">
                        {entity.text && (<FormattedText text={entity.text} />)}
                      </p>
                    </div>
                  </div>
                </div>
          ...
          {
            return (
                <li className="max-w-4xl py-2 px-4 sm:px-6 lg:px-8 mx-auto flex gap-x-2 sm:gap-x-4" key={index}>
                ...
                {entity.image && (<img className="w-full object-cover" src={backendURL+'/images/'+myUsername+'/'+entity.image} alt="Deep Learning" />)}
                {entity.text && (<FormattedText text={entity.text} />)}
                </div>
              </li>        
            );
        ...
        {isChatBlocked && (<li className="max-w-4xl py-2 px-4 sm:px-6 lg:px-8 mx-auto flex gap-x-2 sm:gap-x-4">
                <svg className="shrink-0 size-[38px] rounded-full" width="38" height="38" viewBox="0 0 38 38" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <rect width="38" height="38" rx="6" fill="#2563EB"/>
                  <path d="M10 28V18.64C10 13.8683 14.0294 10 19 10C23.9706 10 28 13.8683 28 18.64C28 23.4117 23.9706 27.28 19 27.28H18.25" stroke="white" stroke-width="1.5"/>
                  <path d="M13 28V18.7552C13 15.5104 15.6863 12.88 19 12.88C22.3137 12.88 25 15.5104 25 18.7552C25 22 22.3137 24.6304 19 24.6304H18.25" stroke="white" stroke-width="1.5"/>
  ...
  <div className="max-w-4xl mx-auto sticky bottom-0 z-10 p-3 sm:py-6">
    <div className="lg:hidden flex justify-end mb-2 sm:mb-3">
      <button type="button" className="p-2 inline-flex items-center gap-x-2 text-xs font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50 focus:outline-none focus:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-white dark:hover:bg-neutral-800 dark:focus:bg-neutral-800" aria-haspopup="dialog" aria-expanded="false" aria-controls="hs-application-sidebar" aria-label="Toggle navigation" data-hs-overlay="#hs-application-sidebar">
        <svg className="shrink-0 size-3.5" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="3" x2="21" y1="6" y2="6"/><line x1="3" x2="21" y1="12" y2="12"/><line x1="3" x2="21" y1="18" y2="18"/></svg>
        <span>Sidebar</span>
      </button>
    </div>
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center">
            <button type="button" className="inline-flex shrink-0 justify-center items-center size-8 rounded-lg text-gray-500 hover:bg-white focus:z-10 focus:outline-none focus:bg-white dark:text-neutral-500 dark:hover:bg-neutral-700 dark:focus:bg-neutral-700">
              <svg className="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="3" rx="2"/><line x1="9" x2="15" y1="15" y2="9"/></svg>
            </button>
