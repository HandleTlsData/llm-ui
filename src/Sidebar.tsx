...
import { ChatMessage, createNewChat, deleteChat, getAllChats, getChatMessages } from './API';
...
type SidebarTextProps = 
{
    ...
    setActiveChatHistory: React.Dispatch<React.SetStateAction<ChatMessage[]>>;
    chatList: number[]; 
    setChatList: React.Dispatch<React.SetStateAction<number[]>>;
}

export const Sidebar = ({activeChatID, setActiveChatID, setActiveChatHistory, chatList, setChatList}: SidebarTextProps)  =>
{
    ...
    <div id="hs-application-sidebar" className="hs-overlay [--auto-close:lg] hs-overlay-open:translate-x-0 -translate-x-full duration-300 transform hidden fixed top-0 start-0 bottom-0 z-[60] w-64 bg-white border-e border-gray-200 overflow-y-auto lg:block lg:translate-x-0 lg:end-auto lg:bottom-0 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:bg-gray-300 dark:[&::-webkit-scrollbar-track]:bg-neutral-700 dark:[&::-webkit-scrollbar-thumb]:bg-neutral-500 dark:bg-neutral-900 dark:border-neutral-700" role="dialog" tabIndex={-1} aria-label="Sidebar">
        <nav className="size-full flex flex-col">
            ...
            <a className="flex-none focus:outline-none focus:opacity-80" aria-label="Preline">
            ...
            <ul className="space-y-1.5 p-4">
                <li onClick={(e)=>{createChatHandler();e.preventDefault();}}>
                <a className="flex items-center gap-x-3 py-2 px-3 text-sm text-gray-700 rounded-lg hover:bg-gray-100 focus:outline-none focus:bg-gray-100 dark:text-neutral-400 dark:hover:bg-neutral-900 dark:hover:text-neutral-300 dark:focus:bg-neutral-900 dark:focus:text-neutral-300">
            ...
            <div className="py-2.5 px-7">
                ...
                <p className="inline-flex items-center gap-x-2 text-xs text-gray-800 dark:text-white">
            ...
            <div className="p-4 border-t border-gray-200 dark:border-neutral-700">
                <a className="flex justify-between items-center gap-x-3 py-2 px-3 text-sm text-gray-700 rounded-lg hover:bg-gray-100 focus:outline-none focus:bg-gray-100 dark:hover:bg-neutral-900 dark:text-neutral-400 dark:hover:text-neutral-300 dark:focus:text-neutral-300" >
                {myUsername}
...
{
    inOverlay: boolean;
    setInOverlay: React.Dispatch<React.SetStateAction<boolean>>;
}


export const SidebarButton = ({inOverlay, setInOverlay}: SidebarButtonProps)  =>
{
    const sidebarSwitch = () => 
    {
        const el = document.getElementById('hs-application-sidebar');
    ...
    <div className="lg:hidden flex justify-end mb-2 sm:mb-3">
        ...
        {inOverlay && (<div onClick={sidebarSwitch} id="hs-application-sidebar-backdrop" data-hs-overlay-backdrop-template="" style={style} className="hs-overlay-backdrop transition duration fixed inset-0 bg-gray-900 bg-opacity-50 dark:bg-opacity-80 dark:bg-neutral-900 "></div>)}
